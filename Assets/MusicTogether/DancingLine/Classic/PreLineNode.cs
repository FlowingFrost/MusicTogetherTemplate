...
namespace MusicTogether.DancingLine.Classic
{
    public class PreLineNode : SerializedMonoBehaviour, ILineNode
    {
        ...
        public MotionType NodeMotionType => endPhysicsState.NodeMotionType;
        ...
        public MotionState UpdatePosition(double time)
        {
            ...
            time = time > 0 ? 0 : time;
            var ms = direction.GetLineHeadMotionState(endPosition, time);
            ms.ParentSpacePosition += PhysicsHelper.CalculateDisplacement(endVelocity, gravity, (float)time);
            return ms;
        }
