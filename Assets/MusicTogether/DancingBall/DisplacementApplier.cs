...
namespace MusicTogether.DancingBall
{
    public class DisplacementApplier : MonoBehaviour
    {
        public void ApplyDisplacement(Map targetMap,List<Block> targetBlocks,Vector3 startPosition, Quaternion previousRotation)
        {
            ...
            if(!mapData.GetBlockData(rootBlockGlobalIndex, out var blockData)) return;
            ...
            // 1.需要点击转向：
            // Left/Right + None：           10/20 在当前水平面内分别向左/右转90度  RootTile:BTM
            // Left/right + Up：             11/21 左右转90°+上转90°             RootTile:BTM + backward
            // Forward + None：              30    跳跃，继续向前                 RootTile:BTM 后2个：不显示任何方块
            // 2.不需要点击
            ...
            switch (displacementID)
            {
                ...
                Jump:
                    ...
                    //然后把起跳点之后的2个block设置为空地板
                    var tileHoldersToApplyEmpty = targetBlocks.Where(b => b.globalBlockIndex <= rootBlockGlobalIndex + 2 && b.globalBlockIndex > rootBlockGlobalIndex).Select(b => b.tileHolder).ToList();
